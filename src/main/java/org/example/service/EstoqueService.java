    ...
    public class EstoqueService {
        ...
        public boolean atualizarQtdEstoque(int quantidadeEstoque, String codigo) throws SQLException {
            String sql = " UPDATE produto SET quantidadeEstoque = ? WHERE codigo = ?";
            try (Connection conn = ConexaoBD.getConnection();
                 PreparedStatement smtm = conn.prepareStatement(sql)) {
                smtm.setInt(1, quantidadeEstoque);
                smtm.setString(2, codigo);
                int linhasAfetadas = smtm.executeUpdate();
                return linhasAfetadas > 0;
            }
        ...
        public int valortotalestoque() throws SQLException {
            String sql = "SELECT SUM(quantidadeEstoque * precoVenda) AS total_Estoque FROM produto";
            try (Connection conn = ConexaoBD.getConnection();
                 PreparedStatement stmt = conn.prepareStatement(sql);
                 ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    return rs.getInt("total_Estoque");
                }
            }
            return 0;
        }
